...
def codivide_gmm(loss, stats_log, epoch, net, p_threshold, targets, 
                targets_clean, codivide_log):
    '''
    ...
    prob_benchmark, ccgmm = ccgmm_probabilities(loss, stats_log, epoch, net, targets, log=False)
...
def codivide_ccgmm(loss, stats_log, epoch, net, p_threshold, targets, 
                    targets_clean, codivide_log):
    '''
        Computes co-divide following two different policies to print/log the comparison between the two.
        The main_policy is used to follow the execution and the benchmark_policy is discarded.
    ...
    prob, ccgmm = ccgmm_probabilities(loss, stats_log, epoch, net, targets)
    prob_benchmark, gmm = gmm_probabilities(loss, stats_log, epoch, net, targets, log=False)
