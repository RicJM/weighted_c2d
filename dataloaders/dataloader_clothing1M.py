...
class clothing_dataset(Dataset):
    def __init__(
        ...
        if mode == "all":
            ...
            self.train_imgs = sample_traning_set(
                train_imgs, self.noisy_labels, num_class, num_samples
            )
            ...
            for i, img_path in enumerate(paths[num_samples:]):
                if img_path in self.clean_labels:
                    ...
                    predicted_clean.append(
                        self.clean_labels[img_path] == self.noisy_labels[img_path]
                    )
            # print('Curr epoch clean/noisy intersection: {}'.format(len(predicted_path)))
            if len(predicted_path) > 0:
                predicted_prob, predicted_clean = np.array(predicted_prob), np.array(
                    predicted_clean
                )
                ...
                tp, fp, fn = (
                    (
                        np.equal(predicted_pred, predicted_clean)
                        * (predicted_clean == 1)
                    ).sum(),
                    (
                        np.not_equal(predicted_pred, predicted_clean)
                        * (predicted_clean == 0)
                    ).sum(),
                    (
                        np.not_equal(predicted_pred, predicted_clean)
    ...
    def __getitem__(self, index):
        if self.mode == "labeled":
            ...
            img_path = self.train_imgs[index]
            target = (
                self.noisy_labels[img_path]
                if not self.clean_all
                else self.clean_labels[img_path]
            )
...
class clothing_dataloader:
    def __init__(
        self, root, batch_size, num_batches, num_workers, log, stronger_aug=False
    ):
        ...
        self.warmup_samples = (
            self.num_batches * self.batch_size * 4
            if stronger_aug
            else self.num_batches * self.batch_size * 2
        )
    ...
    def run(self, mode, pred=[], prob=[], paths=[]):
        if mode == "warmup":
            ...
            test_dataset = clothing_dataset(
                self.root, transform=self.transform_test, mode="test", log=self.log
            )
            ...
            val_dataset = clothing_dataset(
                self.root, transform=self.transform_test, mode="val", log=self.log
            )
